...
@description This class also belongs to package blo.';
	...
	my ($firstLine, $fieldNameParts) = VisDoc::JavadocParser->_getFieldNameParts($text);
...
@description Second description field';
	...
	my $expected = 'This is the first line. This also belongs to the description. Second description field';
	print("RES=$result.\n")     if $debug;
	print("EXP=$expected.\n") if $debug;
	$this->assert( $result eq $expected );	
}
...
#print("fields=" . Dumper($fields));
	...
	}
	{
		...
		my $expected = '<a href="http://java.sun.com/j2se/1.5.0/docs/tooldocs/windows/javadoc.html">javadoc specification</a>';
		print("RES=$result.\n")     if $debug;
		print("EXP=$expected.\n") if $debug;
		$this->assert( $result eq $expected );
	}
...
#print("fields=" . Dumper($fields));
	...
	}
	{
		# sends field 1
		my $result = $fields->[1]->{label};
		my $expected = 'onLoadProgress(name:String, total:Number, loaded:Number, this:Loader) during loading';
		print("RES=$result.\n")     if $debug;
		print("EXP=$expected.\n") if $debug;
		$this->assert( $result eq $expected );
	}
...
';
	...
	my $result = $javadoc->getDescription();
	my $expected =
	  '1111%VISDOC_STUB_TAG_CODE_1% 3333';
...
';
	...
	my $result = $javadoc->getDescription();
	my $expected =
	  '1111%VISDOC_STUB_TAG_IMG_1%3333';
...
sub test_tag_inheritDoc {
	...
	*/
	...
	
	{
		...
		my $result = $javadoc->getDescription();
		my $expected = '<div class="inheritDoc">Description from class Shape. <span class="inheritDocLink"><a href="Shape.html#retainCount">&rarr;</a></span></div>';
