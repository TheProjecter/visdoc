...
sub test_formatParameterData_as2 {
    ...
    
    {
		...
		my $type = 'Canvas';
		my $value = 'null';
		my $parameterData = VisDoc::ParameterData->new($varArgs, $name, $type, $value);

		my $formatter = VisDoc::MemberFormatterFactory::getMemberFormatterForLanguage('as2');
...
sub test_formatParameterData_as3 {
    ...
    
    {
		...
		my $formatter = VisDoc::MemberFormatterFactory::getMemberFormatterForLanguage('as3');
		my $result = $formatter->formatParameterData($parameterData);
		my $expected = 'drawing:Canvas = null';
		print("RES=$result.\n")     if $debug;
		print("EXP=$expected.\n") if $debug;
...
sub test_formatParameterData_java {
    ...
    
    {
		...
		my $formatter = VisDoc::MemberFormatterFactory::getMemberFormatterForLanguage('java');
		my $result = $formatter->formatParameterData($parameterData);
...
#print("methods=" . Dumper($methods));
	...
	}
	{
		...
		my $formatter = VisDoc::MemberFormatterFactory::getMemberFormatterForLanguage('as3');
		my $result = $formatter->typeInfo($methodData);
		my $expected = '(data:BinaryTreeObject = null, key:Number, ...rest) : void';
		print("RES=$result.\n")     if $debug;
		print("EXP=$expected.\n") if $debug;
		$this->assert( $result eq $expected );
	}
