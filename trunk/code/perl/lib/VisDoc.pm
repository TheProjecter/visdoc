...
sub validateFile {
	...
	return { valid => $valid, modificationDate => $fileData->{modificationDate}, language => $fileData->{language} };
}
...
sub writeData {
	...
	my $processing = {
		'classes' => {
			xsltRef => _getXslt('class', $xsltDir, $inPreferences->{xslTemplateForClasses}),
	...
	foreach my $fileData ( @{$inCollectiveFileData} ) {
		...
		my $xmlData = VisDoc::OutputFormatter::formatFileData( $fileData, $inPreferences );
		my $key = 'classes';
		# store
	...
	if ($inPreferences->{generateIndex}) {
		...
		{
			# main.html
			my $xmlData = _createMainHtmlPageXmlData( $dirInfo->{dir}->{html}, $inPreferences, $inCollectiveFileData );
			my $key = 'main';
		...
		my $processOverviewPage = sub {
			...
			my $formatter = $inFormatterName->new($inPreferences, $inCollectiveFileData);
		...
		&$processOverviewPage('VisDoc::XMLOutputFormatterOverviewTree', 'overview-tree');
		&$processOverviewPage('VisDoc::XMLOutputFormatterAllClassesFrame', 'classes');
		&$processOverviewPage('VisDoc::XMLOutputFormatterAllMethodsFrame', 'methods');
		&$processOverviewPage('VisDoc::XMLOutputFormatterAllConstantsFrame', 'constants');
		&$processOverviewPage('VisDoc::XMLOutputFormatterAllPropertiesFrame', 'properties');
		&$processOverviewPage('VisDoc::XMLOutputFormatterAllDeprecatedFrame', 'deprecated');
		...
		if ($packagesCount > 0) {

			{
				...
				my $formatter = VisDoc::XMLOutputFormatterIndex->new($inPreferences, 'index');
				my $xmlData = $formatter->format();
			...
			}
			{
				...
				my $formatter = VisDoc::XMLOutputFormatterIndex->new($inPreferences, 'toc-frame');
				my $xmlData = $formatter->format();
			...
			{
				# overview-frame.html
				my $formatter = VisDoc::XMLOutputFormatterTocPackages->new($inPreferences, undef, $inCollectiveFileData, $tocNavigationKeys);
				my $xmlData = $formatter->format();
				my $key = 'toc-packages';
				push (@{$processing->{$key}->{xmls}}, $xmlData) if $xmlData;
				push (@htmlSupportingFileNames, $xmlData->{uri}) if $xmlData;
			...
			{
				# empty-frame.html
				my $formatter = VisDoc::XMLOutputFormatterEmptyFrame->new($inPreferences, $inCollectiveFileData);
				my $xmlData = $formatter->format();
				my $key = 'empty-frame';
				push (@{$processing->{$key}->{xmls}}, $xmlData) if $xmlData;
				push (@htmlSupportingFileNames, $xmlData->{uri}) if $xmlData;
			...
			foreach my $fileData (@{$inCollectiveFileData}) {
				foreach my $package (@{$fileData->{packages}}) {
					my $formatter = VisDoc::XMLOutputFormatterPackageToc->new($inPreferences, $fileData->{language}, $package);
					my $xmlData = $formatter->format();
					my $key = 'package-toc';
					push (@{$processing->{$key}->{xmls}}, $xmlData) if $xmlData;
					push (@htmlSupportingFileNames, $xmlData->{uri}) if $xmlData;
			...
			{
				# toc.html
				my $formatter = VisDoc::XMLOutputFormatterToc->new($inPreferences, undef, $inCollectiveFileData, $tocNavigationKeys);
				my $xmlData = $formatter->format();
				my $key = 'toc';
				push (@{$processing->{$key}->{xmls}}, $xmlData) if $xmlData;
				push (@htmlSupportingFileNames, $xmlData->{uri}) if $xmlData;
	...
	while ( my ($key, $value) = each %{$processing}) {				
		...
		foreach my $xml ( @{$value->{xmls}} ) {
			# write XML
			_writeXmlDataFile( $dirInfo->{dir}->{xml}, "$xml->{uri}.xml", $xml->{textRef} ) if $inPreferences->{saveXML};

			# write HTML
			my $htmlRef = _transformXmlToHtml( $xml->{textRef}, $value->{xsltRef} );
			_cleanupHtml($htmlRef);
			_writeHtmlFile( $dirInfo->{dir}->{html}, "$xml->{uri}.html", $htmlRef );
		}
		
	}
	return ($dirInfo->{dir}->{html}, $indexHtml, \@htmlDocFileNames, \@htmlSupportingFileNames);
}
...
sub _createWriteDirectories {
    ...
    $dir = _createSubDirectory( $inDocDirectory, $base, 'css' ) if $inPreferences->{copyCSS};
    $info->{dir}->{css} = $dir;
...
sub _copyCss {
	...
	my $cssTemplateDir = $inPreferences->{'cssTemplateDirectory'} || $VisDoc::Defaults::SETTINGS->{'cssTemplateDirectory'};
	
	my $fileName = $inPreferences->{cssTemplate}
	  || $VisDoc::Defaults::SETTINGS->{'cssTemplate'};
	
	my $path = File::Spec->rel2abs( "$cssTemplateDir/$fileName", $inPreferences->{base} );
    ...
    my $dir = File::Spec->rel2abs( $cssTemplateDir,
        $inPreferences->{base} );
...
sub _getXslt {
    ...
    if ($inXsltFile) {
        ...
        $xsltFile = $VisDoc::Defaults::SETTINGS->{'xslTemplateForClasses'} if ($inType eq 'class');
        $xsltFile = $VisDoc::Defaults::SETTINGS->{'xslTemplateForIndexFrameset'} if ($inType eq 'index-frameset');
        $xsltFile = $VisDoc::Defaults::SETTINGS->{'xslTemplateForPackagesFrameset'} if ($inType eq 'packages-frameset');
        $xsltFile = $VisDoc::Defaults::SETTINGS->{'xslTemplateForPackagesTocFrameset'} if ($inType eq 'packages-toc-frameset');
    }
...
'<span id="twistyTOC_toggle" class="twistyContent"><!----></span><div id="toggleTOC_off"';
    ...
    # does not exist yet
    my $formatter = VisDoc::XMLOutputFormatterMainPage->new($inPreferences, $inCollectiveFileData);
