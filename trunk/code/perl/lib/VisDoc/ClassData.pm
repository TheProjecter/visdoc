...
sub getMemberCount {
	...
	$memberType |= ($MEMBER_TYPE->{METHOD} | $MEMBER_TYPE->{PROPERTY} | $MEMBER_TYPE->{INNERCLASS});

    my $count = 0;
    if ($listPrivate) {
    	$count += scalar @{ $this->{methods} }      if $this->{methods} && ($memberType & $MEMBER_TYPE->{METHOD});
    	$count += scalar @{ $this->{properties} }   if $this->{properties} && ($memberType & $MEMBER_TYPE->{PROPERTY});
    	$count += scalar @{ $this->{innerClasses} } if $this->{innerClasses} && ($memberType & $MEMBER_TYPE->{INNERCLASS});
    } else {
		...
		if ($this->{innerClasses} && ($memberType & $MEMBER_TYPE->{INNERCLASS})) {
...
sub getInstanceMethods {
    ...
    my @instanceMethods = grep {
        $_->{type} & $VisDoc::MemberData::TYPE->{'INSTANCE_MEMBER'}
          && !( $_->{type} &
            $VisDoc::MemberData::TYPE->{'CONSTRUCTOR_MEMBER'} )
    } @{ $this->{methods} };
