...
my $macrosPattern;
#our $stubCounter = 0;
our $linkDataRefs; # hash of references to LinkData objects, with key: STUB_INLINE_LINK and value: LinkData ref
...
BEGIN {
    ...
    %functionTags = (
        ...
        $VisDoc::StringUtils::STUB_CODE_BLOCK => { fn => \&_parseStubCode, },
        $VisDoc::StringUtils::STUB_TAG_LITERAL => { fn => \&_parseLiteralText, },
        $VisDoc::StringUtils::STUB_INLINE_LINK   => { fn => \&_parseStubInlineLink, },
    );
...
sub getContents {
    ...
    my $text = $inText;
    $this->handleContentsOfVerbatimTags($text);
...
Some stubs strings need further processing. For instance, a link will be formatted differently than code text.
...
sub _expandMacro {
    ...
    if ( defined( $functionTags{$inTagName}->{fn} ) ) {
        my $owner = $functionTags{$inTagName}->{owner} || $this;
        
        my $result = &{ $functionTags{$inTagName}->{fn} }( $owner, $inTagString,
            $inTagName, $inNumber );
        return $result;
    }
    else {
...
handleContentsOfVerbatimTags( $text ) -> $text

=cut

sub handleContentsOfVerbatimTags {
...
sub _equalizeLeftIndent {
    ...
    foreach my $line (@tmplines) {
		...
		push @lines, $line;
		if ($2) {
			$smallestLeftIndent = $leftIndent if ($leftIndent < $smallestLeftIndent);
		}
    }
...
rest				= <pre>something else</pre> all the rest.</p>
 ...
 4)
 arthur.clemens@xs4all.nl <b>yo!</b>
 beforeFirstLineTag	= 
 first line			= arthur.clemens@xs4all.nl <b>yo!</b> 
 rest				= 
...
sub createLinkReferencesInMemberDefinitions {
	...
	my $event = VisDoc::FindLinksEvent->new($VisDoc::FindLinksEvent::NAME, $this, $inClasses, \&createLinkDataForClass);
...
createLinkDataForClass ( $value ) -> $stub


=cut

sub createLinkDataForClass {
	my ($this, $inClass) = @_;

	my $counterRef = \$this->getStubCounter();
	my $stub = VisDoc::StringUtils::getStubKey( $VisDoc::StringUtils::STUB_INLINE_LINK, $$counterRef );
	$$counterRef++;
	
	my $linkData = $this->createAndStoreInlineLinkData($inClass, $stub);
	$linkData->{class} = $inClass;
...
sub createAndStoreInlineLinkData {
    my ( $this, $inValue, $inStub ) = @_;

    my $linkData = VisDoc::LinkData::createLinkData('link', $inValue, $inStub);
