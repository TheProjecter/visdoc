	...
	foreach my $field (@{$inLinkFields}) {
		if ($this->{$field}) {
			foreach my $class ( @{$inEvent->{classes}} ) {
				...
				# replace
				$this->{$field} =~ s/(\b$class->{name}\b)/$inEvent->{source}->$callback($1)/e;
			}
		}
	}
}
...
sub typeString {
    ...
    push( @type, 'CONSTRUCTOR_MEMBER' ) if ( $inType & $TYPE->{CONSTRUCTOR_MEMBER} );
...
sub as_string {
    ...
    $str .= "\n\t access=" . join(',', @{$this->{access}})       if defined $this->{access};
    $str .= "\n\t memberOrder=$this->{memberOrder}"       if defined $this->{memberOrder};
