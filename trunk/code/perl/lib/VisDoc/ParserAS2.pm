...
sub _parseMethodParameterData {
    ...
    foreach my $paramString ( @{$inList} ) {
        ...
        my VisDoc::ParameterData $paramData =
          VisDoc::ParameterData->new( $varArgs, $name, $dataType, $defaultValue );
...
sub _splitOutMultiplePropertiesOnOneLine {
    ...
    if ( scalar @matches ) {
        my $stub = $this->_copyAttributesOfPropertiesOnOneLine( \@matches, $patternMap );
