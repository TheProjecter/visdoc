...
sub new {
    my ( $class, $inPreferences, $inLanguage, $inData, $inTocNavigationKeys ) = @_;

    my VisDoc::XMLOutputFormatterTocBase $this = $class->SUPER::new($inPreferences, $inLanguage, $inData);
...
sub _writeListData {
    ...
    # sort classes
    @{$classes} =
          sort { lc($a->getClasspathWithoutName()) cmp lc($b->getClasspathWithoutName()) } @{$classes};
...
sub _writeTocNavigation {
	...
	if ($keys->{'overview-tree'}) {
		&$callToWriteLink( 'tree_link', $VisDoc::XMLOutputFormatterOverviewTree::URI );
	} else {
		&$callToWriteName( 'tree_link' );
	}

	if ($keys->{'classes'}) {
		&$callToWriteLink( 'all_classes_link', $VisDoc::XMLOutputFormatterAllClassesFrame::URI );
	} else {
		&$callToWriteName( 'all_classes_link' );
	}
	
	if ($keys->{'methods'}) {
		&$callToWriteLink( 'all_methods_link', $VisDoc::XMLOutputFormatterAllMethodsFrame::URI );
	} else {
		&$callToWriteName( 'all_methods_link' );
	}
	
	if ($keys->{'constants'}) {
		&$callToWriteLink( 'all_constants_link', $VisDoc::XMLOutputFormatterAllConstantsFrame::URI );
	} else {
		&$callToWriteName( 'all_constants_link' );
	}
	
	if ($keys->{'properties'}) {
		&$callToWriteLink( 'all_properties_link', $VisDoc::XMLOutputFormatterAllPropertiesFrame::URI );
	} else {
		&$callToWriteName( 'all_properties_link' );
	}
	
	if ($keys->{'deprecated'}) {
		&$callToWriteLink( 'all_deprecated_link', $VisDoc::XMLOutputFormatterAllDeprecatedFrame::URI );
	} else {
