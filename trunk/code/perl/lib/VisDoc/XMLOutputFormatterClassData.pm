package VisDoc::XMLOutputFormatterClassData;
...
sub _writeAccessKeyLinks {
    ...
    $this->_writeAccessKeyLinkForSection($inWriter, 'summary', 'header_summary') if $this->{data}->getMemberCount( $this->{preferences}->{listPrivate} ) > 0;
...
sub _writeKindOfClass {
    ...
    my $classAccess = $formatter->formatClassAccess( $this->{data}->{type}, $this->{data}->{access}, 1 );
    $inWriter->cdataElement( 'item', $classAccess );
...
sub _writeSummary_typeInfo {
    ...
    my $memberType = $inMember->isa("VisDoc::PropertyData") ? $this->_docTerm('memberproperty_property') : '';
    ...
    if ( $typeInfoString && $typeInfoString ne '' ) {
    	$typeInfoString = $this->{data}->{fileData}->getContents($typeInfoString);
...
sub _writeMembers {
    ...
    &$callToCreateMemberSection('instanceproperties', 'header_instanceproperties');
...
sub _writeMembers_forMemberGroup {
	...
	if ( $members && scalar @{$members} ) {
		...
		foreach my $member ( @{$members} ) {
			$this->_writeMembers_forMemberGroup_memberText($inWriter, $member);
		}
...
sub _writeMembers_forMemberGroup_memberText_fullMethod {
	...
	$memberString =~ s/\n/\n$linefiller/g;
 	...
 	if ( $inMember->isa("VisDoc::PropertyData") ) {
 		my $getSetString = $memberFormatter->getSetString($inMember, $this->_docTerm('read'), $this->_docTerm('write'));
...
sub _writeMembers_forMemberGroup_memberText_fields {
	...
	$this->_writeParamFields($inWriter, 'param', $paramFields) if $paramFields;
